...
export default function BingoGrid({ categories, disabled = false, filledCells, onCellClick, shakingCell }: Props) {
  return (
          ...
          >
            {filled ? (
              <div className="relative w-full h-full flex flex-col items-center justify-center">
                <img src={filled.imagePath} alt={filled.name} className="w-full h-full object-cover rounded" />
                <div className="absolute top-1 left-1 bg-white bg-opacity-80 text-black text-xs px-1 rounded">{filled.name}</div>
              </div>
            ) : (
              category
            )}
